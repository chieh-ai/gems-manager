...
var TARGET_FOLDER_NAME = 'Google Gem Reference';
var DRIVE_FOLDER_ID = ''; // 💡 保持空字串，系統會自動在雲端硬碟建立/搜尋同名資料夾
...
function getLastUpdatedTime() {
  var lastUpdatedStr = "2026-06-25 15:00"; // 安全備用時間
  try {
 ...
 */
function getGemList() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SPREADSHEET_SHEET_NAME);
...
function uploadFileToDriveEx(base64Data, fileName) {
  try {
    ...
    else { folder = DriveApp.createFolder(TARGET_FOLDER_NAME); }
